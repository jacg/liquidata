...
+ `mp.select_field('result')` extracts the interesting item from the compound
  flow. In the `liquidata` version this was done in the previous step, by
  avoiding the use of `>> put.result`
...
TODO use freedom of ordering action, get and put to emphasize action in longer pipelines

```python
action * get.<inputs> >> put.<outputs>
get.<inputs> * action >> put.<outputs>
```
